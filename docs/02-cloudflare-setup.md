...
เปิดที่อยู่นี้ในเบราว์เซอร์ได้เลย ไม่ต้องใช้ Shortcut และไม่ต้องพิมพ์คำสั่งอะไร

```
URL ของ Worker + /test?key= + INGEST_SECRET
```

ระบบจะส่งรูปทดสอบ (สี่เหลี่ยมไล่สีฟ้า) พร้อมข้อความกำกับ เข้าทุกปลายทางใน `LINE_TARGET_IDS`

- ✅ **รูปเด้งเข้า LINE** = ทุกอย่างฝั่งเซิร์ฟเวอร์ทำงานครบแล้ว เหลือแค่ต่อ Shortcut
- ❌ **ไม่เด้ง** = ดูข้อความ error ที่เบราว์เซอร์ตอบกลับ แล้วเทียบกับ [ขั้นที่ 4](04-troubleshooting.md)

ข้อดีของการทดสอบด้วย `/test` คือมันแยกปัญหาให้ชัด — ถ้า `/test` ผ่านแต่ Shortcut ไม่ผ่าน แปลว่าปัญหาอยู่ที่ Shortcut ไม่ใช่ที่เซิร์ฟเวอร์หรือ LINE

> ถ้าอยากทดสอบด้วยรูปจริงและมีเครื่องที่รันคำสั่งได้
>
> ```bash
